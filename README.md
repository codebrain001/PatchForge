...
PatchForge is a **research prototype** demonstrating the potential of LLM-powered agentic systems in physical-world applications. All measurements generated by the system — width, height, thickness, and derived geometry — are **estimates, not precise engineering values**. They are produced by a combination of computer vision models and large language model reasoning, and should be treated as approximations.

The primary goal of this project is to show that:

- **No CAD expertise is required** — the entire pipeline from a photo to a printable 3D mesh is driven by generative AI, with zero manual modelling.
- **LLM agents can reason about the physical world** — calibrating scale from reference objects, arbitrating between competing measurement strategies, and making consensus decisions across a multi-stage pipeline.

For production or safety-critical use, always verify dimensions with physical measurement tools (callipers, rulers) before printing. The generated STL patches are starting points intended to be validated and refined, not drop-in replacements.
